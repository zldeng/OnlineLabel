...
public class PosViterbi implements Runnable
{
	...
	/**
	 * POS Tagger for a segmented sentence
	 * 
	 * @param sentence
	 *            segmented sentence
	 * @param predLabel
	...
	public void pos(Vector<String> sentence, Vector<String> predLabel) throws UnsupportedEncodingException
	{
		...
		for (int i = 0; i < wordArray.length; i++)
			wordArray[i] = FullCharConverter.half2Fullchange(wordArray[i]);
		pos(wordArray, predLabel);
	}

	protected double pos(String[] sentence, Vector<String> predLabel) throws UnsupportedEncodingException
	{
	...
	/**
	 * load resource
	 * when testing,we need load model, POS dictionary, cluster file and
	 * initialize charType
	 * 
	 * @param modelFile
	 * @param dicFile
