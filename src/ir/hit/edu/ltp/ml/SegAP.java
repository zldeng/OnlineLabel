...
public class SegAP extends SegViterbi implements Callable<Object[]>
{
	...
	public void segAPRetrain(String trainingFile, String oldModelFile, String newModelFile, String dicFile, int iterator, String devFile, final int threadNum, final double compressRatio)
			throws Exception
	{
		if (threadNum <= 0)
			throw new Exception("thread number must a positive integer");
		if (1 == threadNum)
			segAPRetrainWithOneThread(trainingFile, oldModelFile, newModelFile, dicFile, iterator, devFile,
					compressRatio);
		else
			segAPRetrainWithMultiThread(trainingFile, oldModelFile, newModelFile, dicFile, iterator, devFile,
					threadNum, compressRatio);
	}

	private void segAPRetrainWithOneThread(String trainingFile, String oldModelFile, String newModelFile, String dicFile, int iterator, String devFile, final double compressRatio)
			throws Exception
	{
		Logger logger = Logger.getLogger("seg");
		...
		SegAP baseSeg = new SegAP(oldModel, segDic, allLabel);
		double[] basePerformance = baseSeg.evalSeg(devFile, 0);
		double basePrecision = basePerformance[0];
		double baseRecall = basePerformance[1];
		double baseF1 = (2 * basePrecision * baseRecall) / (basePrecision + baseRecall);
		logger.info("base model performance P: " + basePrecision + " R: " + baseRecall + " F: " + baseF1);
		...
		for (int it = 0; it < iterator; it++)
		{
			...
			for (int index = 0; index < instanceList.size(); index++)
			{
				...
				SegInstance tmpInst = instanceList.elementAt(index);
				String[] predLabel = new String[tmpInst.label.length];
				...
				model.update(segPipeList.elementAt(index).feature, predPipe.feature);
			...
			SegAP tmpSeg = new SegAP(tmpModel, segDic, allLabel);
			logger.info("evaluate sub-model with dev file...");
			double[] performance = tmpSeg.evalSeg(devFile, it);
			double precision = performance[0];
			double recall = performance[1];
			double F1 = (2 * precision * recall) / (precision + recall);
			logger.info("the evaluation result of sub-model P: " + precision + " R: " + recall + " F: " + F1 + "\n");
			logger.info("writer sub-model to file...\n");

			//merge new model and existing model
			OnlineLabelModel mixModel = MyTools.mixModel(tmpModel, F1, oldModel, baseF1);

			SegAP mixSeg = new SegAP(mixModel, segDic, allLabel);
			logger.info("Evaluate for dev file with mixModel...");
			performance = mixSeg.evalSeg(devFile, it);
			precision = performance[0];
			recall = performance[1];
			logger.info("the evaluation result of mixModel P: " + precision + " R: " + recall + " F: "
					+ (2 * precision * recall) / (precision + recall) + "\n");
			mixModel.writerModel(newModelFile + "-" + it, 0);
	...
	private void segAPRetrainWithMultiThread(String trainingFile, String oldModelFile, String newModelFile, String dicFile, int iterator, String devFile, final int threadNum, final double compressRatio)
			throws Exception
	{
		Logger logger = Logger.getLogger("seg");
		logger.info("start train with training instance...");
		...
		for (int it = 0; it < iterator; it++)
		{
			...
			logger.info("finiash merging parameter!");
			...
			if (compressRatio > 0)
			{
				OnlineLabelModel compressedModel = OnlineLabelModel.loadModel(newModelFile + "-subModle-" + it);
				tmpSeg = new SegAP(compressedModel, segDic, allLabel);
				logger.info("Evaluate for dev file with compressed model...");
				performance = tmpSeg.evalSeg(devFile, it);
				precision = performance[0];
			...
			SegAP mixSeg = new SegAP(mixModel, segDic, allLabel);
			logger.info("Evaluate dev file with mixModel...");
			performance = mixSeg.evalSeg(devFile, it);
			precision = performance[0];
			recall = performance[1];
			logger.info("the evaluation result of mixModel P: " + precision + " R: " + recall + " F: "
			...
			if (compressRatio > 0)
			{
				OnlineLabelModel compressedModel = OnlineLabelModel.loadModel(newModelFile + "-" + it);
				tmpSeg = new SegAP(compressedModel, segDic, allLabel);
				logger.info("Evaluate for dev file with compressed model...");
				performance = tmpSeg.evalSeg(devFile, it);
				precision = performance[0];
